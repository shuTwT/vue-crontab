 ...
 * 
 * 功能描述：
 * 提供 Cron 表达式中分钟字段的配置界面，支持四种配置模式：
 * 1. 每分钟：使用通配符 *
 * 2. 周期：指定一个时间范围，如 5-10 表示第5分钟到第10分钟
 * 3. 间隔：从某分钟开始，每隔多少分钟执行一次，如 0/5 表示从0分钟开始，每5分钟执行一次
 * 4. 指定：手动选择具体的分钟数，如 1,5,10 表示第1、5、10分钟
 * 
 * 取值范围：0-59
 * 允许的通配符：, - * /
 */
 ...
 * @property init - 初始的分钟字段值，如 "*"、"5-10"、"0/5"、"1,5,10"
 */
...
/**
 * 组件 Emits 定义
 * @event updata - 当分钟字段值变化时触发，返回字段名称和新的值
 */
